...
public class Ejercicio4 {
	...
	private static void informacion(List<Integer> lista1, Integer numero) {
		...
		/* Imprime por consola la cantidad de números pares que tiene la lista 1*/
		...
		/* Imprime en consola un arreglo con los números impares*/
		System.out.println("... " + impares.toString());
		
		int p = lista1.size() / 2;
		
		/* Devuelve por consola la posición donde está el valor que represanta la mitad
		 * del largo de la Lista 1. Siendo una lista de 10 elementos, la mitad es 5, y la
		 * posición del elemento 5 es la 2 (Todo arreglo comienza con el índice 0) */
		...
		/* En este caso realiza la impresion del else if ya que evalua cuantos números
		 * de la Lista 1 son mayores que 10 (nro que se pasa por parámetro). En esta lista
		 ...
		 * 2 > 0, siendo esta respuesta verdadera por lo que imprime esa línea (Cabe
		 * aclarar que al ser las 3 impresiones idénticas no tiene sentido la evaluacion) */
		if (c > lista1.size() / 2) {
			System.out.println("...");
		} else if (c > 0) {
			System.out.println("...");
